# Cross-platform · Developer Tools

243 extensions · [← Cross-platform](../README.md)

## Code & Collaboration

| Topic | Extensions |
| --- | --- |
| [Git & Version Control](./git-version-control.md) | 12 |
| [Issue Tracking & Projects](./issue-tracking-projects.md) | 4 |
| [Code, Snippets & Text Utilities](./code-snippets-text-utilities.md) | 26 |
| [Search & Reference](./search-reference.md) | 21 |

## Build, Ship & Operate

| Topic | Extensions |
| --- | --- |
| [Cloud, Hosting & Infrastructure](./cloud-hosting-infrastructure.md) | 25 |
| [Databases](./databases.md) | 2 |
| [APIs & Networking](./apis-networking.md) | 14 |
| [Monitoring & Logs](./monitoring-logs.md) | 6 |

## Platforms & Ecosystems

| Topic | Extensions |
| --- | --- |
| [AI & LLM Tools](./ai-llm-tools.md) | 26 |
| [Web & Frontend](./web-frontend.md) | 16 |
| [Mobile & App Development](./mobile-app-development.md) | 1 |
| [Web3 & Blockchain](./web3-blockchain.md) | 3 |

## Tooling & Workflow

| Topic | Extensions |
| --- | --- |
| [Terminal & Editors](./terminal-editors.md) | 9 |
| [Package & Dependency Tools](./package-dependency-tools.md) | 12 |
| [Automation & Scripting](./automation-scripting.md) | 4 |
| [Files & Transfer](./files-transfer.md) | 8 |
| [Design & Assets](./design-assets.md) | 9 |

Plus [General](./general.md) — 45 extensions that don't fit a topic yet.
